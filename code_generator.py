...
def generateScales(interrupt_frequency,board='UNO',clock_frequency=16e6):
    clock_resolution = 1/clock_frequency
    output = {}
    for timer,res in timers[board].items():
        output[timer] = []
        for value in prescalers:
...
def generatePrescaleCode(timer,prescaler):
    preSclRegs = []
    for i in range(2,-1,-1):
        preSclRegs.append("CS{}{}".format(timer,i))

    foo = list(compress(preSclRegs, clock_bits[prescaler]))
    bar = " | ".join(["(1 << {})".format(item) for item in foo])
    return bar

def generateCode(timer,prescalar,count_val):
    setup_code = template.format(
        timer = timer,
        prescale_str = generatePrescaleCode(timer,prescalar),
        count_val = count_val,
        ctc_string = ctc_strings[timer],
        prescalar = prescalar
        )
...
def Main(ISR_Freq,board='UNO'):
    ...
    options = generateScales(ISR_Freq)
    pprint(options)
    timer = int(input("Enter Selected Timer: "))
    prescalar = int(input("Enter Selected Prescalar: "))
    code = generateCode(timer,prescalar,dict(options[timer])[prescalar])
    print(code)
